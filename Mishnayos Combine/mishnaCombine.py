...
class MishnayosCombiner:
    def __init__(self, root):
        ...
        self.metadata_preview = tk.StringVar(value="No files added yet")
        ...
        # Update metadata preview when export name changes
        self.export_name.trace_add("write", lambda *args: self.update_metadata_preview())
    ...
    def update_metadata_preview(self):
        """Update the metadata preview based on current files and export name"""
        try:
            if not self.file_entries:
                self.metadata_preview.set("No files added yet")
                return

            # Get metadata from first file
            from mutagen.easyid3 import EasyID3
            from mutagen.id3 import ID3NoHeaderError
            first_file = self.file_entries[0].file_path
            metadata = {}

            try:
                audio_tags = EasyID3(first_file)
                # Copy over common metadata fields
                metadata_fields = ['artist', 'album', 'genre', 'date', 'composer', 'albumartist']
                for field in metadata_fields:
                    value = audio_tags.get(field, [None])[0]
                    if value:
                        metadata[field] = value
            except ID3NoHeaderError:
                pass

            # Calculate the title (same logic as in export)
            ...
            preview_parts = []
            if metadata:
                metadata_str = ', '.join([f"{k.title()}: '{v}'" for k, v in metadata.items()])
                preview_parts.append(metadata_str)
            preview_parts.append(f"Title: '{export_title}'")

            self.metadata_preview.set(' | '.join(preview_parts))

        except Exception as e:
            self.metadata_preview.set(f"Error reading metadata: {str(e)}")
    ...
    def create_widgets(self):
        ...
        metadata_frame = tk.Frame(bottom_frame)
        metadata_frame.pack(fill=tk.X, pady=(0, 10))

        tk.Label(metadata_frame, text="Metadata preview:").pack(side=tk.LEFT)
        metadata_label = tk.Label(metadata_frame, textvariable=self.metadata_preview,
                                 font=("Arial", 9), fg="blue", anchor="w")
        metadata_label.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
    ...
    def update_export_name(self, file_path):
        ...
        try:
            ...
            if masechet_match and perek_match and rabbi_match:
                new_name = f"Masechet {masechet_match} Perek {perek_match} Harav {rabbi_match} M"
        ...
        # Update metadata preview after export name changes
        self.update_metadata_preview()
    ...
    def _process_export(self, export_path, progress_window, file_entries):
        ...
        try:
            ...
            try:
                ...
                # Copy all metadata fields from first file
                for field, value in metadata.items():
                    audio[field] = value
                audio['title'] = export_title
                audio.save()
                metadata_str = ', '.join([f"{k}='{v}'" for k, v in metadata.items()])
                print(f"Set metadata: {metadata_str}, Title='{export_title}'")
