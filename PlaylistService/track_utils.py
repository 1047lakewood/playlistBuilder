...
class TrackUtils:
    ...
    def update_track_metadata(track: Track):
        ...
        track.duration = (
            metadata["duration"] if metadata["duration"] != 0 else track.duration
        )
            ...
            with open(file_path, "rb") as f:
                ...
                if file_ext == ".mp3":
                    try:
                        ...
                        duration = (
                            round(audio_mp3.info.length, 2)
                            if audio_mp3.info and hasattr(audio_mp3.info, "length")
                            else 0.0
                        )
                        ...
                        if audio_mp3.tags:
                            ...
                            if "TPE1" in audio_mp3.tags:
                                ...
                                if (
                                    frame.text
                                    and len(frame.text) > 0
                                    and frame.text[0].strip()
                                ):
                        ...
                        if not artist or artist == "Unknown Artist":
                            artist = (
                                "Unknown Artist"  # Or some other placeholder/derivation
                            )
                        ...
                        logging.debug(
                            f"Could not read MP3 metadata for {file_path} with MP3(fileobj): {e}"
                        )
                        if (
                            not title or title == "Unknown Title"
                        ):  # Ensure title fallback
                            title = os.path.splitext(os.path.basename(file_path))[0]
                    except (
                        Exception
                    ) as e:  # General fallback for other unexpected errors
                        logging.error(
                            f"Unexpected error reading MP3 metadata for {file_path}: {e}"
                        )
                        if (
                            not title or title == "Unknown Title"
                        ):  # Ensure title fallback
                    ...
                    # Secondary attempt using generic parser if tags were not found
                    if artist == "Unknown Artist" or title == "Unknown Title":
                        try:
                            audio_generic = File(file_path, easy=True)
                            if audio_generic:
                                artist = audio_generic.get("artist", [artist])[0].strip()
                                title = audio_generic.get("title", [title])[0].strip()
                        except Exception as e_fallback:
                            logging.debug(
                                f"Additional MP3 metadata read failed for {file_path}: {e_fallback}"
                            )
                    ...
                    try:
                        ...
                        duration = (
                            round(audio.info.length, 2) if audio and audio.info else 0.0
                        )
                        ...
                        try:
                            ...
                            logging.debug(
                                f"Could not read ID3 from WAV {file_path}: {e_id3}"
                            )
                            if title == "Unknown Title":
                                title = os.path.splitext(os.path.basename(file_path))[0]
                    except Exception as e:
                        logging.debug(
                            f"Could not read WAV metadata for {file_path}: {e}"
                        )
                        ...
                        try:
                            ...
                            duration = (
                                round(audio_full.info.length, 2)
                                if audio_full and audio_full.info
                                else 0.0
                            )
                        except Exception as e2:
                            logging.debug(
                                f"Fallback ID3/File read failed for WAV {file_path}: {e2}"
                            )
                            if title == "Unknown Title":
                                title = os.path.splitext(os.path.basename(file_path))[0]
                    ...
                    try:
                        ...
                        logging.debug(
                            f"Could not read WMA metadata for {file_path}: {e}"
                        )
                        if title == "Unknown Title":
                            title = os.path.splitext(os.path.basename(file_path))[0]

                elif file_ext in [".m4a", ".mp4", ".aac", ".m4v"]:
                    try:
                        audio = MP4(fileobj=f)
                        if "\xa9ART" in audio:  # Artist
                            ...
                            artist = (
                                str(audio["----:com.apple.iTunes:Artist"][0])
                                .decode("utf-8", "ignore")
                                .strip()
                            )
                        ...
                        if "\xa9nam" in audio:  # Name/Title
                            ...
                            title = (
                                str(audio["----:com.apple.iTunes:Name"][0])
                                .decode("utf-8", "ignore")
                                .strip()
                            )
                        elif (
                            "----:com.apple.iTunes:Title" in audio
                        ):  # Less common but possible
                            title = (
                                str(audio["----:com.apple.iTunes:Title"][0])
                                .decode("utf-8", "ignore")
                                .strip()
                            )
                        ...
                        logging.debug(
                            f"Could not read M4A/MP4/AAC metadata for {file_path}: {e}"
                        )
                        if title == "Unknown Title":
                            title = os.path.splitext(os.path.basename(file_path))[0]

                    # Additional attempt if tags were not found
                    if artist == "Unknown Artist" or title == "Unknown Title":
                        try:
                            generic = File(file_path, easy=True)
                            if generic:
                                artist = generic.get("artist", [artist])[0].strip()
                                title = generic.get("title", [title])[0].strip()
                        except Exception as e_fallback:
                            logging.debug(
                                f"Additional MP4 metadata read failed for {file_path}: {e_fallback}"
                            )
                    ...
                    try:
                        ...
                        logging.debug(
                            f"Could not read generic metadata for {file_path}: {e}"
                        )
                        if title == "Unknown Title":
                            title = os.path.splitext(os.path.basename(file_path))[0]
            ...
            if file_ext == ".mp3":
                ...
                try:
                    ...
                    try:
                        ...
                        logging.info(
                            f"Initial EasyID3 failed: {str(e)}. Trying to add tags..."
                        )
                        ...
                        try:
                            ...
                            if audio is None:
                                raise ValueError(
                                    f"Unsupported or corrupt file: {track.path}"
                                )
                            ...
                            if hasattr(audio, "add_tags") and callable(audio.add_tags):
                                try:
                                    ...
                                    logging.debug(
                                        f"Could not add tags with File: {str(e2)}"
                                    )
                        except Exception as e3:
                            logging.debug(
                                f"File approach failed: {str(e3)}. Trying with MP3..."
                            )
                        ...
                        try:
                            ...
                            logging.debug(
                                f"MP3 approach failed: {str(e4)}. Last resort..."
                            )
                            ...
                            try:
                                ...
                                raise ValueError(
                                    f"Could not create or access ID3 tags: {track.path}"
                                )
                    ...
                    try:
                        ...
                        id3.save(track.path)
                        logging.info(
                            f"Successfully updated MP3 metadata using direct ID3: {track.path}"
                        )
                    except Exception as e2:
                ...
                try:
                    ...
                    id3.save(track.path)
                    logging.info(
                        f"Successfully wrote ID3 tags to WAV file: {track.path}"
                    )
                except Exception as e:
                    logging.debug(
                        f"Failed to write metadata to WAV file {track.path}: {str(e)}"
                    )
