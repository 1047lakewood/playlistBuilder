            ...
            with open(file_path, 'rb') as f:
                ...
                if file_ext == '.mp3':
                    try:
                        ...
                        logging.warning(f"Could not read MP3 metadata for {file_path} with MP3(fileobj): {e}")
                    ...
                    try:
                        ...
                        try:
                            ...
                            logging.warning(f"Could not read ID3 from WAV {file_path}: {e_id3}")
                            if title == "Unknown Title":
                                title = os.path.splitext(os.path.basename(file_path))[0]
                    except Exception as e:
                        logging.warning(f"Could not read WAV metadata for {file_path}: {e}")
                        ...
                        try:
                            ...
                            logging.warning(f"Fallback ID3/File read failed for WAV {file_path}: {e2}")
                            if title == "Unknown Title":
                                title = os.path.splitext(os.path.basename(file_path))[0]
                    ...
                    try:
                        ...
                        logging.warning(f"Could not read WMA metadata for {file_path}: {e}")
                        if title == "Unknown Title":
                            title = os.path.splitext(os.path.basename(file_path))[0]
                    ...
                    try:
                        ...
                        logging.warning(f"Could not read M4A/MP4/AAC metadata for {file_path}: {e}")
                        if title == "Unknown Title":
                            title = os.path.splitext(os.path.basename(file_path))[0]
                    ...
                    try:
                        ...
                        logging.warning(f"Could not read generic metadata for {file_path}: {e}")
                        if title == "Unknown Title":
                            title = os.path.splitext(os.path.basename(file_path))[0]
    ...
    def change_track_metadata(track: Track, artist: str, title: str):
        ...
        try:
            if file_ext == '.mp3':
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if hasattr(audio, 'add_tags') and callable(audio.add_tags):
                                try:
                                    ...
                                    logging.warning(f"Could not add tags with File: {str(e2)}")
                        except Exception as e3:
                            logging.warning(f"File approach failed: {str(e3)}. Trying with MP3...")
                        ...
                        try:
                            ...
                            logging.warning(f"MP3 approach failed: {str(e4)}. Last resort...")
                ...
                logging.warning("WAV files have limited metadata support for writing")
                ...
                try:
                    ...
                    logging.warning(f"Failed to write metadata to WAV file {track.path}: {str(e)}")
